...
public class VerEndActivity extends BaseActivity implements AdapterView.OnItemClickListener {
    ...
    ListView lsHomeTask;
    private List<TaskListEntity> end;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ver_end);
        ButterKnife.bind(this);
        end = (List<TaskListEntity>) getIntent().getSerializableExtra(
                "end");
        chooseALL.addAll(end);
        adapter = new TaskListAdapter(VerEndActivity.this, R.layout.item_task_list_content, chooseALL);
    ...
    private class OnItemSelectedListenerImpl implements AdapterView.OnItemSelectedListener {
        ...
        private void listAllChange() {
            if (chooseALL.size() == 0 || chooseALL.size() == end.size() / 2) {
                chooseALL.clear();
                chooseALL.addAll(end);
            }
            chooseList.clear();
